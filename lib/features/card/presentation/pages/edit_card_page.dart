...
import 'package:mobile_pizzaria_app/features/card/presentation/components/main_card_button.dart';
...
class _EditCardPageState extends State<EditCardPage> {
  ...
  void update() {
    ...
    final MyCard updatedCard = MyCard(
      id: widget.card.id,
      cardName: cardName,
    ...
    cardCubit.updateCard(updatedCard);
  ...
  void setMainCard() {
    ...
    profileCubit.updateProfile(
      ProfileUser(
        ...
        selectedCardId: widget.card.id,
        selectedAddressId: currentProfileUser.selectedAddressId,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Cartão definido com sucesso!'),
      ),
    );
  }
    ...
    return BlocConsumer<CardCubit, CardState>(
      builder: (context, state) {
        if (state is CardLoaded) {
          return _buildUpdatePage(state.cards);
        } else if (state is CardLoading) {
          return const LoadingPage();
        } else {
          return const Scaffold(
            body: Center(
              child: Text("Cartão não criado"),
            ),
          );
        }
      },
  ...
  Widget _buildUpdatePage(List<MyCard> cards) {
    return Scaffold(
      appBar: AppBar(
              ...
              children: [
                ...
                MyTextField(
                  controller: cardNameController
                    ..text = cards[widget.index].cardName,
                ...
                MaskedTextField(
                  controller: cpfController
                    ..text = cpfMaskFormatter.maskText(cards[widget.index].cpf),
                  hintText: 'CPF',
                  obscureText: false,
                ...
                MaskedTextField(
                  controller: cardNumberController
                    ..text = cardMaskFormatter
                        .maskText(cards[widget.index].cardNumber),
                    ...
                    Expanded(
                      child: MaskedTextField(
                        controller: expireDateController
                          ..text = cards[widget.index].expireDate,
                    ...
                    Expanded(
                      child: MaskedTextField(
                        controller: cvvController
                          ..text = cvvMaskFormatter
                              .maskText(cards[widget.index].cvv),
                        hintText: 'CVV',
                        obscureText: false,
